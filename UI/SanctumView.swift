...
struct SanctumView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $activeRealm) { realm in
            ...
            case .forge:
                ForgeView()
            case .observatory:
                EternalObservatoryView()
            }
        }
        .task { viewModel.startLiveRefresh() }
    ...
    private func presence(_ vm: SanctumViewModel, accent: Color) -> some View {
        HStack(spacing: 10) {
            ...
            Text(vm.livingStatus).font(.subheadline).foregroundStyle(PersonaTheme.mercurySilver).lineLimit(1)
            Spacer(minLength: 8)
            Text(vm.activePersonaID.uppercased()).font(.caption2.weight(.bold)).tracking(1.4).foregroundStyle(accent)
        }
    ...
    private func realms(accent: Color) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 10) {
                SanctumRealmCard(title: "Forge", subtitle: "Create", symbol: "flame", accent: PersonaTheme.forgeEmber) { activeRealm = .forge }
                SanctumRealmCard(title: "Eternal", subtitle: "Observe", symbol: "eye", accent: PersonaTheme.eternalViolet) { activeRealm = .observatory }
            }
        }
    }
    ...
    private func environment(_ vm: SanctumViewModel, accent: Color) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 8) {
                ...
                SanctumSigil(title: "HEALTH", value: "\(vm.overallHealthScore)", accent: PersonaTheme.healthColor(vm.overallHealthScore))
            }
        }
    }
    ...
    private func insightCard(_ insight: Insight, accent: Color) -> some View {
        VStack(alignment: .leading, spacing: 9) {
            HStack {
                ...
                Spacer()
                Image(systemName: "waveform.path.ecg").font(.caption).foregroundStyle(accent.opacity(0.75))
            }
    ...
    private func emptyWhisper(accent: Color) -> some View {
        VStack(alignment: .leading, spacing: 7) {
            Text("NEXUS IS LISTENING").font(.caption2.weight(.bold)).tracking(1.6).foregroundStyle(accent)
            Text("No new signal has crossed the chamber.").font(.subheadline).foregroundStyle(.secondary)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(16)
    ...
    private func sectionLabel(_ title: String, accent: Color) -> some View {
        HStack(spacing: 8) {
            Rectangle().fill(accent.opacity(0.45)).frame(width: 18, height: 1)
    ...
    private func invocationBar(accent: Color) -> some View {
        HStack(spacing: 0) {
            SanctumRitualButton(symbol: "bubble.left.and.bubble.right", label: "Invoke") { showAsk = true }
            SanctumRitualButton(symbol: "brain.head.profile", label: "Memory") { showMemory = true }
            SanctumRitualButton(symbol: "book.closed", label: "Codex") { showCodex = true }
        }
        .padding(.horizontal, 18)
        .padding(.vertical, 10)
